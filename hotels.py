...
def process_coordinates(coordinates):
  ...
  for coord in coordinates:
    coord_str = coord[0]  # Extract the coordinate string from the list
    lat, long = map(
      convert_to_decimal_degrees,
      coord_str.split(','))  # Split the coordinate string by comma
    ...
    if hotels:
      ...
      print("No hotels found for coordinates:", coord_str)
  return hotel_details


def read_coordinates():
  with open('coords.csv', 'r', encoding='utf_8_sig') as file:
...
def main():
  ...
  download_from_aws('coords.csv', 'coords.csv',
                    bucket_name)  # Download coordinates from S3 bucket
  ...
  with open('hoteldata.csv', 'w', newline='') as file:
    ...
    writer.writerow(
      ["Highest Price", "Lowest Price", "Median Price", "Highest Price Hotel"])
    writer.writerows(hotel_details)
  upload_to_aws('hoteldata.csv', bucket_name,
                'hoteldata.csv')  # Upload hotel data to S3 bucket
