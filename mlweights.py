...
def preprocess_data(data):
    data['Vessel Density'] = data['Registered Vessels'] / data['County Population']
    data['Slip Scarcity'] = data['Registered Vessels'] / data['Number of Slips']
    data['Relative Population Growth'] = data['County Population Growth'] / data['County Population']
    data['Home Value to Income Ratio'] = data['Median Home Value'] / data['Average Household Income']

    data.drop(columns=['Registered Vessels', 'Number of Slips', 'County Population', 'County Population Growth', 'Median Home Value', 'Average Household Income'], inplace=True)
...
def optimize_weights():
    ...
    return jsonify({'message': f'Optimization completed and weights saved to AWS. Time taken: {int(hours)}h {int(minutes)}m {int(seconds)}s'})
